...
# Function to save audio results
...
def check_environment_conditions():
    ...
    if humidity is not None and temperature is not None:
        ...
        # Define normal ranges for climate conditions
        normal_temp_range = (20.0, 25.0)  # Normal temperature range in °C
        normal_humidity_range = (30.0, 60.0)  # Normal humidity range in %

        if not (normal_temp_range[0] <= temperature <= normal_temp_range[1]):
            print("Warning: The temperature is outside the normal range for the climate!")
        if not (normal_humidity_range[0] <= humidity <= normal_humidity_range[1]):
            print("Warning: The humidity is outside the normal range for the climate!")
    else:
        print("Failed to get temperature and humidity reading. Try again!")
...
def main():
    ...
    while True:
        ...
        # Check temperature and humidity conditions if baby is detected
        if baby_detected:
